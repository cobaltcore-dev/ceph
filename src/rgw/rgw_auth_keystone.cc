                           ...
                           std::list<rgw::keystone::TokenEnvelope::Role> roles)
{
  ...
  return [allowed_items, token_roles=std::move(roles)](const rgw::auth::Identity::aclspec_t& aclspec) {
    ...
    for (const auto& r : token_roles) {
      if (r.is_system_reader && r.is_admin) {
        /*
         * System scope reader defeats project-level permissions, so this
         * grant is added regardless of what the aclspec matched above.
         */
...
{
  /* Shares make_keystone_acl_strategy() with TokenEngine so S3 requests
   * authenticated via Keystone honor the same Swift-format ACL entries. */
  return keystone_acl_strategy_from_token(token);
}
